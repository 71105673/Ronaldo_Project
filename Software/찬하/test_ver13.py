...
def main():
    ...
    try:
        ...
        success_sound = pygame.mixer.Sound("./sound/SIUUUUU.wav")   # 얘는 막았을 때 소리 -> 구해야함
    except pygame.error as e:
    ...
    try: 
        # 한글 파일명은 로드 오류를 일으킬 수 있으므로 영어로 변경합니다.
        # 실제 파일 이름도 'success.gif'로 변경해주세요.
        success_gif = cv2.VideoCapture("./image/final_ronaldo/에밀신_얼굴.gif")
    except Exception as e: 
        print(f"GIF 로드 오류: success.gif - {e}"); success_gif = None
        ...
        if screen_state["current"] == "game":
            ...
            if active_gif:
                ...
                if gif_frame is not None:
                    ...
                    if is_success:
                        # 성공 GIF는 작게 (예: 화면 너비의 1/3)
                        gif_display_size = (screen_width // 3, screen_height // 3) 
                    elif is_failure:
                        # --- 수정: 실패 GIF는 원래대로 전체 화면으로 표시 ---
            ...
            text_lines_1p = [
                "<1 Player>", "", "MODE : Goalkeeper",
                "- 장갑 착용", "- 화면에 표시되는 10가지 영역 중 한 곳 선택",
                "- 시간 제한 5초",
            ]
            text_lines_2p = [
                "<2 Players>", "", "MODE : Kicker / Goalkeeper 중 선택",
                "", "Kicker", "- 샬라샬라 공 차세요~~",
                "", "Goalkeeper", "- 장갑 착용",
                "- 화면에 표시되는 10가지 영역 중 한 곳 선택", "- 시간 제한 5초"
            ]
            
            y_start = title_rect.bottom + 100
            x_offset_1p = screen_width // 4 - 400
            x_offset_2p = screen_width // 2 + 100
