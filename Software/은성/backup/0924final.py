...
def main():
    game_state = {
        ...
        "is_failure": False,
        "is_success": False,
        "attacker_did_not_kick": False,
        "synthesized_frame_index": 0,
        "synthesized_last_update": 0,
        "synthesis_info": None
    }
            ...
            if elapsed < 5000:
                ...
                if game_state["final_col"] is None:
                    ...
                    if game_state["game_mode"] == 'multi' and game_state["attacker_selected_col"] is None:
                        ...
                        game_state["is_success"] = False # 골키퍼는 막을 공이 없었으므로 성공은 아님
                        game_state["is_failure"] = False # 골키퍼가 실점한 것도 아님
                        # ball_col을 None으로 두어 공이 화면에 나타나지 않게 함
                    else:
                        # 기존 로직 (싱글 플레이 또는 공격수가 선택한 경우)
