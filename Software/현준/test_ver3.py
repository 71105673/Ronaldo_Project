...
try:
    ...
    print(f"효과음 로드 오류: {e}")
    button_sound = pygame.mixer.Sound(buffer=b'')
    siu_sound = pygame.mixer.Sound(buffer=b'')
...
class ImageButton:
    ...
    def draw(self, screen):
        if self.is_hovered:
            draw_rect = self.hover_image.get_rect(center=self.rect.center)
            screen.blit(self.hover_image, draw_rect)
        else:
            screen.blit(self.image, self.rect)

    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN and self.is_hovered:
            button_sound.play() ### 버튼 클릭 시 효과음 재생 ###
            if self.action:
                self.action()
...
def main():
    ...
    def go_back():
        ...
        target = "menu"
        if current in ["single", "multi", "webcam_view"]: target = "game"
        elif current == "game": target = "menu"
        elif current == "info": target = "menu"
        start_transition(target)

    def set_game_mode(mode):
        siu_sound.play() ### 'single' 또는 'multi' 선택 시 SIU 효과음 재생 ###
        game_mode["mode"] = mode
        if mode == "single": start_transition("webcam_view")
        else: start_transition(mode)

    button_menu = ImageButton("./image/btn_start.png", screen_width - 300, screen_height - 175, 400, 250, lambda: start_transition("game"))
    button_info = ImageButton("./image/btn_desc.png", screen_width - 150, 150, 100, 100, lambda: start_transition("info"))
    button_back = ImageButton("./image/btn_back.png", 150, 150, 100, 100, go_back)
    button_quit = ImageButton("./image/btn_exit.png", screen_width - 150, 150, 100, 100, lambda: start_transition("menu"))
    game_mode = {"mode": None}
    button_single = ImageButton("./image/btn_single.png", screen_width//2 - 280, screen_height//2 + 200, 550, 600, lambda: set_game_mode("single"))
    button_multi = ImageButton("./image/btn_multi.png", screen_width//2 + 430, screen_height//2 + 200, 550, 600, lambda: set_game_mode("multi"))
    
    buttons_by_state = {
        "menu": [button_menu, button_info],
        "game": [button_single, button_multi, button_back],
        "webcam_view": [button_back],
        "multi": [button_back],
        "info": [button_quit]
    }
    ...
    while running:
        for event in pygame.event.get():
            ...
            if not (fading_in or fading_out):
                for button in buttons_by_state.get(screen_state["current"], []):
