...
def main():
    ...
    resources = {
        "cap": cv2.VideoCapture(2), # 골키퍼 카메라
        "cap2": cv2.VideoCapture(1),# 공격수 카메라
        "ser_goalkeeper": None, 
        "ser_attacker": None,   
    ...
    resources["videos"]["failure_gif"] = cv2.VideoCapture("../image/son.mp4")
    ...
    while running:
        ...
        if current_screen in ["menu", "game"]:
            ...
            active_gif = None
            if should_play_gif:
                if game_state["is_failure"]: active_gif = resources["videos"]["failure_gif"]
                elif game_state["is_success"]: active_gif = resources["videos"]["success_gif"]
            ...
            # 4. [수정] GIF가 활성화된 경우, 기존 화면 위에 덧그립니다.
            if game_state["gif_start_time"] and active_gif:
                ...
                if gif_frame is not None:
                    frame_resized = cv2.resize(gif_frame, (main_monitor_width, screen_height), interpolation=cv2.INTER_AREA)
                    frame_rgb = cv2.cvtColor(frame_resized, cv2.COLOR_BGR2RGB)
                    gif_surface = pygame.surfarray.make_surface(frame_rgb.swapaxes(0, 1))
                    screen.blit(gif_surface, (goalkeeper_start_x, 0))
                    if game_state["game_mode"] == "multi":
                        screen.blit(gif_surface, (attacker_start_x, 0))
