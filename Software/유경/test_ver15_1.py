...
GOLD_COLOR = (255, 215, 0) # 최고 기록용 색상
...
try:
    ...
    rank_font = pygame.font.Font("./fonts/netmarbleB.ttf", 100) # 칭호 폰트
...
def main():
    ...
    try:
        ...
        fail_sound = pygame.mixer.Sound("./sound/SIUUUUU.wav")
    except pygame.error as e:
        print(f"효과음 로드 오류: {e}"); button_sound=siu_sound=success_sound=fail_sound=None
        ...
        if screen_state["current"] == "game":
            ...
            if gif_start_time:
                ...
                if ret:
                    ...
                    if countdown_start_time and (pygame.time.get_ticks() - countdown_start_time) < 5000:
                        ...
                        if contours and cv2.contourArea(max(contours, key=cv2.contourArea)) > 500:
                            ...
                            x, y, w, h = cv2.boundingRect(largest); _, cam_w, _ = frame.shape
                    ...
                    if countdown_start_time:
                        ...
                        if elapsed < 5000:
                            ...
                            if final_selected_col is None:
                                final_selected_col, ball_col, chances_left = selected_grid_col, random.randint(0, 4), chances_left - 1
            ...
            text_1p = ["[1인 플레이]", "1. 5초의 카운트 다운이 시작됩니다.", "2. 카메라에 비치는 빨간색 물체를 인식합니다.", "3. 5개의 영역 중 하나를 선택합니다.", "4. 공을 막으면 성공!"]
            text_2p = ["[2인 플레이]", "1. COM과 번갈아가며 공격과 수비를 합니다.", "2. 5번의 기회가 주어집니다.", "3. 더 많은 득점을 한 플레이어가 승리합니다."]
