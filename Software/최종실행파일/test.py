...
# --- 결과 분석 및 파라미터 업데이트 함수 (개선된 버전) ---
def analyze_and_update_params(roi_cb, roi_cr):
    """
    추출된 Cb, Cr 값의 평균과 표준편차를 이용해 최적 범위를 계산하고
    전역 파라미터를 업데이트합니다. 이 방식은 아웃라이어에 더 강건합니다.
    """
    ...
    # --- 핵심 개선 사항: 평균과 표준편차 사용 ---
    # 1. 각 채널의 평균과 표준편차를 계산합니다.
    cb_mean = np.mean(roi_cb)
    cb_std = np.std(roi_cb)
    cr_mean = np.mean(roi_cr)
    cr_std = np.std(roi_cr)

    # 2. 평균을 중심으로 표준편차의 특정 배수만큼 범위를 설정합니다.
    #    multiplier 값을 조절하여 감도(범위의 넓이)를 튜닝할 수 있습니다. (예: 1.5 ~ 2.5)
    multiplier = 2.0
    skin_params['cb_min'] = int(max(0, cb_mean - multiplier * cb_std))
    skin_params['cb_max'] = int(min(255, cb_mean + multiplier * cb_std))
    skin_params['cr_min'] = int(max(0, cr_mean - multiplier * cr_std))
    skin_params['cr_max'] = int(min(255, cr_mean + multiplier * cr_std))
    ...
    ax = plt.gca()
    ax.scatter(roi_cb.flatten(), roi_cr.flatten(), alpha=0.3, s=8, label='Selected Skin Pixels')
    
    # 평균 지점 표시
    ax.plot(cb_mean, cr_mean, 'y*', markersize=15, label='Mean')
    ...
    rect = patches.Rectangle((skin_params['cb_min'], skin_params['cr_min']), 
                             ...
                             linewidth=2, edgecolor='r', facecolor='none', label='Optimal Boundary (Mean ± StdDev)')
    ax.add_patch(rect)
...
def main():
    ...
    cap = cv2.VideoCapture(0) # 0: 기본 카메라, 다른 카메라는 1, 2...
    if not cap.isOpened():
