...
try:
    ...
    print(f"감지된 모니터 정보: {desktop_sizes}")
    print(f"계산된 전체 너비: {total_width}, 최대 높이: {max_height}")
except AttributeError:
    print("pygame.display.get_desktop_sizes()를 사용할 수 없습니다. Info() 객체를 사용합니다.")
    ...
    def draw_webcam_view():
        ...
        if bg_video and (game_state["waiting_for_start"] or game_state["countdown_start"]):
            ...
            if ret_vid:
                ...
                pos_x = (main_monitor_width - new_w) // 2
