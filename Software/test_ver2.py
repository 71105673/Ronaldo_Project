...
# Set up the display to be full screen automatically
...
GRID_COLOR = (0, 255, 0)
HIGHLIGHT_COLOR = (255, 0, 0, 100) # Highlight color (R, G, B, Alpha)
...
# =========================================
# ImageButton Class with Hover Effect
# =========================================
class ImageButton:
...
def main():
    ...
    # Webcam and game state variables
    ...
    if not cap.isOpened():
        print("Error: Could not open webcam."); cap = None
    ...
    try:
        ...
        print(f"Error loading image: Ball.png - {e}"); ball_image = None

    try:
        failure_gif = cv2.VideoCapture("./image/G.O.A.T/siuuu.gif")
    except Exception as e:
        print(f"Error loading GIF: siuuu.gif - {e}"); failure_gif = None
    ...
    while running:
        ...
        if screen_state["current"] == "game":
            ...
            should_play_gif = is_failure and result_display_time and (pygame.time.get_ticks() - result_display_time > 1000)
