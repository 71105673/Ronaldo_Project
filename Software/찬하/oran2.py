...
def main():
    ...
    buttons = {
        "menu": [
            # 기존 'menu' 상태 버튼들은 그대로 유지 (예: 정보 버튼)
            ImageButton("../image/btn_desc.png", main_monitor_center_x*2 - 150, 150, 100, 100, lambda: start_transition("info"), sound=resources["sounds"].get("button"))
            # 여기에 시작 버튼이 있었다면, 이제는 화면 아무 곳이나 클릭해서 "game" 상태로 넘어가도록 변경할 수 있습니다.
        ],
        "game": [
            # ★ 새로 추가된 텍스트 메뉴 버튼 ★
            MenuButton("1인 플레이", main_start_x + 50, 250, 300, 50, font, lambda: start_game("single"), sound=resources["sounds"].get("button")),
            MenuButton("2인 플레이", main_start_x + 50, 310, 300, 50, font, lambda: start_game("multi"), sound=resources["sounds"].get("button")),
            ImageButton("../image/btn_back.png", 150, 150, 100, 100, go_to_menu, sound=resources["sounds"].get("button"))
        ],
        "face_capture": [
            ImageButton("../image/btn_back.png", 150, 150, 100, 100, go_to_game_select, sound=resources["sounds"].get("button"))
        ],
        # ... (이하 다른 상태의 버튼들은 동일) ...
    ...
    def draw_game_screen():
        # "game_bg" 라는 이름으로 저장했던 Ground.jpg 이미지를 배경으로 그립니다.
        if resources["images"].get("game_bg"):
            screen.blit(resources["images"]["game_bg"], (0, 0))
        else:
            screen.fill(BLACK)
        
        # "FC반도체" 같은 타이틀을 추가할 수 있습니다.
        title_surf = title_font.render("FC반도체", True, WHITE)
