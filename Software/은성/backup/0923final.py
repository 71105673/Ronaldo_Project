...
from Button import ImageButton, MenuButton  # 버튼 클래스가 정의된 모듈
from Config import *                        # 게임 설정값(화면 크기, 색상, 폰트 등)이 정의된 모듈

def main():
    # game_state: 게임의 모든 동적 데이터를 저장하는 딕셔너리
    game_state = {
        "screen_state": "menu",         # 현재 화면 상태 
        "chances_left": 5,              # 남은 기회 (골키퍼 기준)
        "score": 0,                     # 골키퍼 점수
        "highscore": load_highscore(),  # 파일에서 불러온 최고 점수
        "attacker_score": 0,            # 공격수 점수 (2인 플레이용)
        "final_rank": "",               # 게임 종료 시 표시될 최종 랭크 또는 결과 메시지
        "end_video": None,              # 종료 화면에서 재생될 배경 비디오 
        "countdown_start": None,        # 라운드 시작 카운트다운이 시작된 시간 
        "selected_col": None,           # 골키퍼가 현재 선택 중인 영역 (0-4)
        "final_col": None,              # 골키퍼가 최종적으로 선택한 영역
        "ball_col": None,               # 공이 날아갈 영역 (랜덤 또는 공격수 선택)
        "is_failure": False,            # 골키퍼가 막기 실패했는지 여부
        "is_success": False,            # 골키퍼가 막기 성공했는지 여부
        "synthesized_frames": [],       # Photofunia로 합성된 GIF 프레임들을 저장하는 리스트
        "synthesized_frame_index": 0,   # 현재 표시할 합성 GIF의 프레임 인덱스
        "synthesized_last_update": 0,   # 합성 GIF 애니메이션의 마지막 프레임 업데이트 시간
        "synthesis_info": None          # 얼굴 합성에 필요한 정보(얼굴 이미지 경로, GIF 경로 등)를 임시 저장
    }

    # 화면 전환(페이드인/아웃) 효과에 사용될 변수들
    transition_surface = pygame.Surface((screen_width, screen_height))  
    transition_surface.fill(BLACK)
    transition_alpha = 0                # transition_surface의 투명도 (0: 투명, 255: 불투명)
    transition_target = None            # 전환될 다음 화면 상태
    transition_speed = 15               # 페이드 효과의 속도
    fading_out = False                  # 현재 페이드 아웃(점점 어두워짐) 중인지 여부
    fading_in = False                   # 현재 페이드 인(점점 밝아짐) 중인지 여부

    resources = {
        "cap": cv2.VideoCapture(2),          # 골키퍼용 카메라
        "cap2": cv2.VideoCapture(0),         # 공격수용 카메라
        "ser_goalkeeper": None,              # 골키퍼용 시리얼 통신 객체
        "ser_attacker": None,                # 공격수용 시리얼 통신 객체
    ...
    try:
        ...
        print("공격수 보드(COM19)가 성공적으로 연결되었습니다.")
    except serial.SerialException as e:
        print(f"오류: 공격수 보드(COM19)를 열 수 없습니다 - {e}")

    # 사운드 및 이미지 파일 로딩 (try-except로 파일 누락 예외 처리)
    try:
        ...
        resources["sounds"]["failed"] = resources["sounds"]["siu"]  # 실패 사운드로 siu 재사용
    except Exception as e:
        print(f"사운드 로딩 오류: {e}")
    try:
        ball_img = pygame.image.load("../image/final_ronaldo/Ball.png").convert_alpha()
        resources["images"]["scoreboard_ball"] = pygame.transform.scale(ball_img, (80, 80)) # 점수판용 작은 공
        resources["images"]["ball"] = pygame.transform.scale(ball_img, (200, 200)) # 게임 화면용 큰 공
        resources["images"]["glove"] = pygame.transform.scale(pygame.image.load("../image/glove.png").convert_alpha(), (200, 200))
    ...
    def reset_game_state(full_reset=True):
        ...
        game_state.update({
            ...
            "synthesized_frames": [], "synthesized_frame_index": 0, "synthesis_info": None,
        })
        # 게임을 완전히 새로 시작할 때만 초기화되는 변수들
        if full_reset:
            game_state.update({
    ...
    def start_game(mode):
        """게임 모드(1인/2인)를 설정하고 얼굴 캡처 화면으로 전환하는 함수"""
    ...
    def go_to_game_select():
        """게임 선택(1인/2인) 화면으로 돌아가는 함수"""
        reset_game_state(full_reset=True)
        start_transition("game")
        
    buttons = {
        "game": [
            MenuButton("1인 플레이", main_start_x + 50, 400, 350, 100, font, lambda: start_game("single"), sound=resources["sounds"].get("button")),
        ...
        "end": [
            ImageButton("../image/btn_restart.png", main_monitor_center_x - 300, screen_height - 250, 400, 250, go_to_game_select, sound=resources["sounds"].get("button")),
    ...
    def draw_menu_or_game_screen(state):
        """메인 메뉴 또는 게임 선택 화면을 그리는 함수 (동일한 배경 비디오 사용)"""
        pygame.draw.rect(screen, BLACK, (goalkeeper_start_x, 0, goalkeeper_monitor_width, screen_height))
        pygame.draw.rect(screen, BLACK, (attacker_start_x, 0, attacker_monitor_width, screen_height))
    ...
    def draw_face_capture_screen():
        ...
        # 얼굴 캡처 UI(안내 문구, 사각형 등)를 그리는 내부 함수
        ...
        if game_state["game_mode"] == "multi":
            if resources["cap2"].isOpened():
                ...
                if ret_cam2:
                    ...
                    frame_cam2_flipped = cv2.flip(frame_cam2, 1) # 좌우 반전 (거울 모드)
        ...
        if not game_state["captured_goalkeeper_face_filename"]:
            ...
            if resources["ser_goalkeeper"] and resources["ser_goalkeeper"].in_waiting > 0:
                ...
                for byte in uart_bytes:
                    header = byte >> 5 # 상위 3비트가 헤더
                    if header == 2: # 헤더가 2이면 얼굴 좌표 데이터
                        game_state["goalkeeper_face_data_buffer"].append(byte & 31) # 하위 5비트가 데이터

            # 4바이트(20비트) 데이터가 모이면 좌표로 변환
            if len(game_state["goalkeeper_face_data_buffer"]) >= 4:
                ...
                x_coord_raw, y_coord_raw = (full_data >> 10) & 0x3FF, full_data & 0x3FF # 각각 10비트씩 분리

                # 원본 좌표와 화면에 맞게 스케일링된 좌표를 저장
                ...
                if capture_area.collidepoint(coords["scaled"]):
                    ...
                    if filename:
                        ...
                        if game_state["game_mode"] == "multi":
                            send_uart_command(resources["ser_attacker"], 'face') # 공격수 캡처 시작
                        else: 
        ...
        if game_state["last_goalkeeper_face_coords"]:
            ...
            new_center = (coords[0], coords[1] - 50) # y좌표 보정
    ...
    def draw_webcam_view():
        ...
        # [공격수 모니터] 2인 플레이 시에만 카메라 화면 및 UI 표시
        ...
        if game_state["game_mode"] == "multi":
            ...
            if game_state["attacker_selected_col"] is not None: # 공격수가 선택한 영역 표시
                pygame.draw.rect(screen, RED, (attacker_start_x + game_state["attacker_selected_col"] * cell_w_atk, 0, cell_w_atk, screen_height), 10)
            if game_state["ball_col"] is not None and resources["images"]["ball"]: # 공이 날아간 위치 표시
            ...
            if elapsed < 5000: # 카운트다운 5초 동안
                ...
                if resources["ser_goalkeeper"] and resources["ser_goalkeeper"].in_waiting > 0:
                    try:
                        ...
                        for byte in uart_bytes:
                            header = byte >> 5
                            if header == 1: # 헤더 1은 그리드 데이터
                                value = byte & 31
                                if 1 <= value <= 5:
                                    game_state["selected_col"] = 5 - value # 수신 값(1~5)을 인덱스(4~0)로 변환
                    except Exception as e:
    ...
    def draw_end_screen():
        ...
        # 2인 플레이와 1인 플레이의 점수 표시 방식 분리
        if game_state["game_mode"] == "multi":
            score_str = f"{game_state['score']} : {game_state['attacker_score']}"
            goalkeeper_text = score_font.render("Goalkeeper", True, BLACK)
            attacker_text = score_font.render("Attacker", True, BLACK)
    ...
    def handle_events():
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):
                running = False
                return
            # 메뉴 화면에서 아무 키나 누르면 게임 선택 화면으로 전환
            if game_state["screen_state"] == "menu" and event.type == pygame.KEYDOWN:
                start_transition("game")
            # 게임 대기 상태에서 스페이스 바를 누르면 카운트다운 시작
        ...
        if current_screen in ["menu"]:
            ...
            if game_state["gif_start_time"] and (pygame.time.get_ticks() - game_state["gif_start_time"] > 3000):
                if game_state["chances_left"] > 0:
                    ...
                    # 합성할 정보(얼굴, GIF 경로 등)가 있다면 '합성 중' 화면으로 전환
                    ...
                    else: # 합성이 필요 없는 경우(e.g., 무승부) 바로 종료 화면으로
            ...
            if not game_state["synthesized_frames"] and game_state["synthesis_info"]:
                ...
                # Photofunia 라이브러리를 호출하여 얼굴 합성된 GIF 프레임 리스트를 생성
