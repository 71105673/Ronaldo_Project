...
def main():
    ...
    def set_game_mode(mode):
        nonlocal countdown_start_time
        game_mode["mode"] = mode
        # --- 수정: 'single'일 때 'webcam_view'로 상태 변경 및 타이머 시작 ---
        if mode == "single":
            screen_state["current"] = "webcam_view"
            countdown_start_time = pygame.time.get_ticks() # 카운트다운 시작 시간 기록
        else:
                ...
                if ret:
                    ...
                    if countdown_start_time is not None:
                        ...
                        if elapsed_time < 1000:
                            ...
                            # 3초가 지나면 타이머를 리셋하여 더 이상 표시되지 않게 함
                            countdown_start_time = None
                        
                        if countdown_text:
                            text_surf = countdown_font.render(countdown_text, True, WHITE)
