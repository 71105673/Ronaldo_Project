...
def main():
    ...
    uart_ball_col = None # 멀티플레이용 골키퍼 위치
    
    last_end_frame = None # 종료 영상 프레임 저장용
    ...
    # 웹캠 초기화 (안정성을 위해 DSHOW 백엔드 사용 권장)
    ...
    ser = None
    try:
        # ❗❗ 중요: 'COM11'를 자신의 Basys3 보드 COM 포트로 변경하세요.
    ...
    while running:
        ...
        if screen_state["current"] == "game":
            ...
            if gif_start_time and active_gif:
                ...
                if ret:
                    ...
                    if countdown_start_time:
                        ...
                        if elapsed < 5000:
                            # ==========================================================
                            # [수정] FPGA 하드웨어 신호 수신으로 객체 인식 로직 대체
                            # ==========================================================
                            
                            # [기존] 파이썬 OpenCV 객체 인식 로직 (이제 사용 안함)
                            ...
                            # [신규] FPGA로부터 UART 신호를 받아 selected_grid_col 업데이트
                            if ser and ser.is_open and ser.in_waiting > 0:
                                try:
                                    # 버퍼에 쌓인 데이터를 모두 읽어 가장 마지막 값만 사용
                                    uart_stream = ser.read(ser.in_waiting).decode('utf-8', errors='ignore').strip()
                                    
                                    # 데이터가 '01', '05'와 같은 2자리 헥스 코드 문자열이라고 가정
                                    if len(uart_stream) >= 2:
                                        last_hex = uart_stream[-2:]
                                        grid_value = int(last_hex, 16) # '01' -> 1
                                        
                                        if 1 <= grid_value <= 5:
                                            selected_grid_col = grid_value - 1 # 1~5 값을 0~4 인덱스로 변환
                                            print(f"FPGA 신호 수신: {last_hex} -> Grid: {selected_grid_col}")
                                        else:
                                            selected_grid_col = None
                                except (ValueError, TypeError) as e:
                                    print(f"UART 데이터 처리 오류: '{uart_stream}'는 유효한 헥스값이 아닙니다.")
                                    selected_grid_col = None
                            
                            # 멀티플레이어 모드일 때 골키퍼 위치 수신 로직 (별도 유지)
                            if game_mode['mode'] == 'multi' and ser and ser.is_open and ser.in_waiting > 0:
                                try:
                                    # 이 부분은 골키퍼의 스위치 입력을 처리하는 로직으로 가정
                                    # FPGA가 키커와 골키퍼 신호를 다르게 보낸다고 가정
                                ...
                                except Exception as e: print(f"UART 골키퍼 데이터 수신 오류: {e}")
            ...
            if end_video_to_play:
                read_new_frame = True
                if end_video_to_play == defeat_video and loop_counter % 2 != 0:
