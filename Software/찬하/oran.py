...
import imageio # Remake.py의 비디오 처리를 위해 추가

# Remake.py의 버튼 클래스를 여기에 통합합니다.
...
def send_uart_command(serial_port, command):
    commands = {
        'grid': 225, 'face': 226, 'kick': 227, 'stop': 0
    }
...
# 3-모니터 레이아웃 설정 (기존 final.py 코드 유지)
...
HOVER_COLOR = (200, 200, 200) # Remake.py에서 추가

try: pygame.mixer.init()
except: pass

# 폰트 로드 함수 수정 (Remake.py 스타일 폰트 추가)
def load_font(path, size, default_size, is_bold=False):
    try:
...
# Remake.py의 폰트 설정 가져오기
font_path = "../fonts/netmarbleM.ttf"
font_path_bold = "../fonts/netmarbleB.ttf"
font_title = load_font(font_path_bold, 80, 90, is_bold=True)
font_large = load_font(font_path, 40, 50)
font_medium = load_font(font_path, 30, 40)
font_small = load_font(font_path, 25, 30)
# final.py 기존 폰트
countdown_font = load_font(font_path, 200, 250)
score_font = load_font(font_path_bold, 60, 70)
rank_font = load_font(font_path_bold, 100, 110)
description_font = load_font(font_path, 50, 60)

# =========================================
# ★ Remake.py의 MenuButton 클래스 가져오기 ★
# =========================================
class MenuButton:
    ...
    def draw(self, surface):
        ...
        pygame.draw.line(surface, WHITE, (self.rect.left, self.rect.bottom), (self.rect.right, self.rect.bottom), 1)
...
# ==========================
def main():
    # ★ 게임 상태를 Remake.py 방식으로 변경 ★
    game_state = {
        "screen_state": "start_screen", # 초기 상태: start_screen
        "chances_left": 5, "score": 0, "highscore": load_highscore(),
        ...
        "gif_frame_index": 0, "waiting_for_start": False, "game_mode": None, "is_capturing_face": False,
        "goalkeeper_face_data_buffer": [], "last_goalkeeper_face_coords": None, "captured_goalkeeper_face_filename": None,
        "attacker_face_data_buffer": [], "last_attacker_face_coords": None, "captured_attacker_face_filename": None,
    ...
    resources = {
        "cap": cv2.VideoCapture(0), "cap2": cv2.VideoCapture(2), "ser_goalkeeper": None, 
        "ser_attacker": None, "sounds": {}, "images": {}, "videos": {}, "gif_frames": {},
        "last_cam_frame": None, "last_cam2_frame": None 
    }
    
    # (final.py의 리소스 로딩 부분 유지)
    if not resources["cap2"].isOpened(): print("경고: 카메라 2(공격수용)를 열 수 없습니다.")
    try:
        resources["ser_goalkeeper"] = serial.Serial('COM17', 9600, timeout=0)
        print("골키퍼 보드(COM17)가 성공적으로 연결되었습니다.")
    except serial.SerialException as e: print(f"오류: 골키퍼 보드(COM17)를 열 수 없습니다 - {e}")
    try:
        resources["ser_attacker"] = serial.Serial('COM13', 9600, timeout=0)
        print("공격수 보드(COM13)가 성공적으로 연결되었습니다.")
    except serial.SerialException as e: print(f"오류: 공격수 보드(COM13)를 열 수 없습니다 - {e}")
    try:
    ...
    # ★ Remake.py의 배경 리소스 로딩 추가 ★
    try:
        # 시작 화면 비디오
        start_video_path = "../image/game_thumbnail.mp4"
        vid_reader = imageio.get_reader(start_video_path, loop=True)
        resources["start_video_frames"] = [pygame.transform.scale(pygame.image.fromstring(f.tobytes(), f.shape[1::-1], "RGB"), (main_monitor_width, screen_height)) for f in vid_reader]
        # 메인 메뉴 배경
        ground_img_path = "../image/Ground.jpg"
        resources["images"]["main_menu_bg"] = pygame.transform.scale(pygame.image.load(ground_img_path).convert(), (main_monitor_width, screen_height))
    except Exception as e:
        print(f"시작 화면/메뉴 배경 로딩 오류: {e}")
        # 오류 발생 시 검은 화면으로 대체
        resources["start_video_frames"] = [pygame.Surface((main_monitor_width, screen_height))]
        resources["images"]["main_menu_bg"] = pygame.Surface((main_monitor_width, screen_height))

    # (final.py의 나머지 리소스 로딩 유지)
    try:
    ...
    if bg_video.isOpened():
        ...
        bg_video_w = int(bg_video.get(cv2.CAP_PROP_FRAME_WIDTH)); bg_video_h = int(bg_video.get(cv2.CAP_PROP_FRAME_HEIGHT))
    ...
    def reset_game_state(full_reset=True):
        game_state.update({
            "countdown_start": None, "selected_col": None, "final_col": None, "ball_col": None,
            "is_failure": False, "is_success": False, "result_time": None, "gif_start_time": None,
            "gif_frame_index": 0, "waiting_for_start": False, "is_capturing_face": False, 
            "attacker_selected_col": None, "goalkeeper_face_data_buffer": [], "last_goalkeeper_face_coords": None,
            "attacker_face_data_buffer": [], "last_attacker_face_coords": None,
        })
        if full_reset:
            game_state.update({"chances_left": 5, "score": 0, "attacker_score": 0, 
                               "captured_goalkeeper_face_filename": None, "captured_attacker_face_filename": None})
    ...
    def go_to_menu():
        reset_game_state(full_reset=True)
        start_transition("main_menu") # "menu" -> "main_menu"로 변경
    
    # ★ 텍스트 메뉴 버튼 생성 (Remake.py 방식) ★

    menu_margin_left = main_start_x + 50 # 중앙 모니터 좌측에서 50px 간격
    menu_buttons = [
        MenuButton("1인 플레이", menu_margin_left, 400, 300, 50, font_large, lambda: start_game("single"), sound=resources["sounds"].get("button")),
        MenuButton("2인 플레이", menu_margin_left, 460, 300, 50, font_large, lambda: start_game("multi"), sound=resources["sounds"].get("button"))
    ]
    # ★ 기존 ImageButton들은 유지하되, menu/game 부분은 제거/수정 ★
    buttons = {
        "face_capture": [ImageButton("../image/btn_back.png", 150, 150, 100, 100, go_to_menu, sound=resources["sounds"].get("button"))],
        "webcam_view": [ImageButton("../image/btn_back.png", 150, 150, 100, 100, go_to_menu, sound=resources["sounds"].get("button"))],
        "info": [ImageButton("../image/btn_exit.png", main_monitor_center_x*2 - 150, 150, 100, 100, go_to_menu, sound=resources["sounds"].get("button"))],
        "end": [ImageButton("../image/btn_restart.png", main_monitor_center_x - 300, screen_height - 250, 400, 250, go_to_menu, sound=resources["sounds"].get("button")),
                ImageButton("../image/btn_main_menu.png", main_monitor_center_x + 300, screen_height - 250, 400, 250, lambda: start_transition("start_screen"), sound=resources["sounds"].get("button"))]
    }
    ...
    # ================================================================= #
    # ★ 화면 그리기 함수들을 Remake.py 스타일로 통합/수정 ★
    # ================================================================= #
    def draw_start_screen():
        nonlocal video_frame_idx
        # 양 옆 모니터는 검게
        ...
        # 중앙 모니터에 비디오 배경
        screen.blit(resources["start_video_frames"][video_frame_idx], (main_start_x, 0))
        video_frame_idx = (video_frame_idx + 1) % len(resources["start_video_frames"])

        # 텍스트 그리기
        kor_rect = start_text_kor_surf.get_rect(center=(main_monitor_center_x, screen_height * 0.8))
        eng_rect = start_text_eng_surf.get_rect(center=(main_monitor_center_x, kor_rect.bottom + 20))
        screen.blit(start_text_kor_surf, kor_rect)
        screen.blit(start_text_eng_surf, eng_rect)
    ...
    def draw_main_menu_screen():
        ...
        # 중앙 모니터에 Ground.jpg 배경
        screen.blit(resources["images"]["main_menu_bg"], (main_start_x, 0))
        
        # "FC반도체" 타이틀
        title_rect = title_surf.get_rect(center=(main_monitor_center_x, 150))
    ...
    # (이하 final.py의 다른 그리기 함수들 유지)
    def draw_player_info(surface, start_x, width, player_type):
        overlay = pygame.Surface((width, screen_height), pygame.SRCALPHA); overlay.fill((0, 0, 0, 100))
        ...
        chances_text = font_large.render("CHANCES", True, WHITE)
    ...
    # (draw_face_capture_screen, draw_webcam_view, draw_info_screen, draw_end_screen 등은 그대로 유지)
    # ... (draw_face_capture_screen 함수 전체) ...
    # ... (draw_webcam_view 함수 전체) ...
    # ... (draw_info_screen 함수 전체) ...
    # ... (draw_end_screen 함수 전체) ...
    # ... (capture_and_save_face 함수 전체) ...
    def draw_face_capture_screen():
        ...
        def draw_capture_ui(surface, start_x, width, center_x, captured_filename, player_name):
            ...
            if not captured_filename:
                ...
                desc_surf = font_large.render("얼굴을 중앙의 사각형에 맞춰주세요", True, WHITE) # 설명 수정
    ...
    def draw_webcam_view():
        ...
        if game_state["waiting_for_start"]:
            ...
            start_text_l2 = font_large.render("(Press Space Bar)", True, WHITE)
    ...
    def handle_events():
        ...
        for event in pygame.event.get():
            ...
            elif current_screen == "webcam_view" and game_state["waiting_for_start"]:
    ...
    while running:
        ...
        if current_screen == "start_screen":
            ...
            # (이하 웹캠 뷰의 GIF 재생 및 라운드 종료 로직은 final.py와 동일)
            if game_state["gif_start_time"] and (pygame.time.get_ticks() - game_state["gif_start_time"] > 3000):
                if game_state["chances_left"] > 0:
                    ...
                    # (게임 종료 로직...)
                    if game_state["game_mode"] == 'multi':
                        if game_state["score"] > game_state["attacker_score"]: game_state["winner"], game_state["final_rank"], game_state["end_video"] = "goalkeeper", "GOALKEEPER WINS!", resources["videos"]["victory"]
                        elif game_state["attacker_score"] > game_state["score"]: game_state["winner"], game_state["final_rank"], game_state["end_video"] = "attacker", "ATTACKER WINS!", resources["videos"]["victory"]
                        else: game_state["winner"], game_state["final_rank"], game_state["end_video"] = "draw", "DRAW", resources["videos"]["defeat"]
                    else:
                        game_state["winner"] = "goalkeeper"
                        if game_state["score"] > game_state["highscore"]: game_state["highscore"] = game_state["score"]; save_highscore(game_state["score"])
            ...
            if should_play_gif and gif_key:
                if not game_state["gif_start_time"]:
                    game_state["gif_start_time"] = pygame.time.get_ticks(); game_state["gif_frame_index"] = 0
