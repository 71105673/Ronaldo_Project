...
def main():
    ...
    try: end_video = cv2.VideoCapture("./video/ending.mp4")
    ...
    def handle_game_logic():
        ...
        # [수정] GIF 재생이 시작되는 이 시점에 사운드를 함께 재생합니다.
        if active_gif and not gif_start_time:
            gif_start_time = pygame.time.get_ticks()
            # 성공/실패에 맞는 사운드를 여기서 재생
            if is_success and success_sound:
                success_sound.play()
            elif is_failure and failed_sound:
                failed_sound.play()

        if gif_start_time:
            screen.fill(BLACK) 
                # [수정] 성공/실패 상태에 따라 다른 속도 제어
            if is_success:
                    # 성공 GIF(pk.gif)는 2 프레임마다 한 번씩 (기존 속도)
                    if loop_counter % 2 == 0:
                    ...
                    # 실패 GIF(siuuu.gif)는 4 프레임마다 한 번씩 (더 느리게)
                    # 이 숫자를 조절하여 실패 GIF의 속도를 바꾸세요.
                    if loop_counter % 4 == 0:
                        ret, frame = active_gif.read()
                        if not ret: active_gif.set(cv2.CAP_PROP_POS_FRAMES, 0); ret, frame = active_gif.read()
                        if ret: gif_frame = frame
