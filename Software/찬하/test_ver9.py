...
def main():
    ...
    ball_cell = None # --- 추가: 공의 위치 ---
    match_success = False # --- 추가: 성공 여부 ---
    ...
    try:
        ...
        ball_image = pygame.transform.scale(ball_image, (200, 200)) # 공 이미지 크기 조절
    except pygame.error as e:
        print(f"이미지 로드 오류: Ball.png - {e}")
        ball_image = None

    try:
        failure_gif = cv2.VideoCapture("./image/G.O.A.T/siuuu.gif")
    except Exception as e:
        print(f"GIF 로드 오류: siuuu.gif - {e}")
        failure_gif = None
    ...
    def go_back():
        # --- 수정: 새로운 게임 상태 변수 초기화 추가 ---
        nonlocal countdown_start_time, selected_grid_cell, final_selected_cell, ball_cell, match_success
    ...
    while running:
        ...
        if screen_state["current"] == "game":
            ...
            # --- 수정: 실패 시 GIF 재생 로직으로 변경 ---
            if match_success and failure_gif:
                ret, frame = failure_gif.read()
                if not ret:
                    failure_gif.set(cv2.CAP_PROP_POS_FRAMES, 0)
                    ret, frame = failure_gif.read()
                if ret:
                    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                    frame = cv2.resize(frame, (screen_width, screen_height))
                ...
                if ret:
                    ...
                    if countdown_start_time is not None:
                        ...
                        if elapsed_time < 5000:
                            ...
                            if final_selected_cell is None:
                                ...
                                # --- 추가: 공 위치 랜덤 생성 및 성공 여부 판단 ---
                                ball_cell = (random.randint(0, 1), random.randint(0, 4))
                                
                                # --- 수정: 일치하지 않을 때 match_success를 True로 설정 ---
                    ...
                    # --- 추가: 공 그리기 ---
                    if ball_cell is not None and ball_image and not match_success:
                        row, col = ball_cell
                        cell_w, cell_h = screen_width / 5, screen_height / 2
