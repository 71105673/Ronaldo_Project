...
def main():
    ...
    def draw_end_screen():
        ...
        # 3. (중간) 캡처된 얼굴 이미지 그리기 (배경 비디오 위에)
        face_img_scaled = None
        if game_state["captured_face_filename"] and os.path.exists(game_state["captured_face_filename"]):
            try:
                face_img = pygame.image.load(game_state["captured_face_filename"]).convert_alpha()
                # 얼굴 이미지 크기를 더 적절하게 조절
                face_img_scaled = pygame.transform.scale(face_img, (int(face_img.get_width() * 2.2), int(face_img.get_height() * 2.2)))
        ...
        # 4. (가장 위) 등급 및 점수 텍스트 그리기 (얼굴 이미지 위에)
        if face_img_scaled:
            face_rect = face_img_scaled.get_rect(center=(main_monitor_center_x, screen_height / 2))
            ...
            rank_y_pos, score_y_pos, highscore_y_pos = screen_height/2 - 150, screen_height/2, screen_height/2 + 80
